...
export const ProductionGantt: React.FC<ProductionGanttProps> = ({ 
  ...
  const minMonth = useMemo(
    () => addMonths(normalizeMonth(minDate), -1),
    [minDate]
  );
  const maxMonth = useMemo(
    () => addMonths(normalizeMonth(maxDate), 1),
    [maxDate]
  );
    ...
    <>
      ...
      <div className="mb-6 overflow-hidden rounded-lg border border-border bg-card shadow-sm">
        <div className="flex items-center justify-between border-b border-border bg-muted px-4 py-3">
          ...
          <div className="flex items-center gap-px rounded border border-border bg-background text-[10px] font-bold">
            {(['daily', 'weekly', 'monthly', 'yearly'] as const).map((view) => (
              <button
                key={view}
                type="button"
                onClick={() => setRateView(view)}
                className={cn(
        ...
        <div className="grid-bg px-4 py-4">
      ...
      {calculatedProjects.length > 0 ? (
        <ContextMenu onOpenChange={(open) => {
          ...
          <ContextMenuTrigger asChild>
            <div ref={ganttChartRef} className="gantt-chart w-full min-w-full rounded-lg border border-border bg-card shadow-sm overflow-hidden">
              ...
              <div
                className="gantt-body relative bg-background gantt-lines"
        ...
        <ContextMenu>
          <ContextMenuTrigger asChild>
            <div ref={ganttChartRef} className="p-4 text-center border border-border rounded-lg bg-card shadow-sm text-foreground" style={{minHeight: '200px'}}>
